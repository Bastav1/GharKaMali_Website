...
export default function ProductClient({ slug, initialProduct }: Props) {
  ...
  useEffect(() => {
    ...
    // Fallback: fetch client-side if no server product
    setLoading(true);
    getShopProducts({ limit: 100 })
      .then((all: any) => {
        const list: any[] = Array.isArray(all) ? all : [];
  ...
  return (
    <>
      ...
      <div style={{ paddingTop: 'var(--nav-h)', minHeight: '100svh', background: 'var(--bg)' }}>
        ...
        {/* Breadcrumb */}
        <nav aria-label="Breadcrumb" style={{ background: '#fff', borderBottom: '1px solid var(--border)', padding: '12px 0' }}>
            ...
            <div style={{ display: 'flex', borderBottom: '1px solid var(--border)', overflowX: 'auto', scrollbarWidth: 'none' }}>
              {DETAIL_TABS.map(tab => (
                <button
                  ...
                  style={{ padding: '18px 28px', fontWeight: 700, fontSize: '0.88rem', border: 'none', background: 'transparent', cursor: 'pointer', whiteSpace: 'nowrap', color: activeTab === tab.id ? 'var(--forest)' : 'var(--text-2)', borderBottom: activeTab === tab.id ? '2.5px solid var(--forest)' : '2.5px solid transparent', transition: 'all 0.2s', fontFamily: 'inherit' }}
                >
                  {tab.label}
                </button>
              ))}
            ...
            <div style={{ padding: '32px' }}>
              {activeTab === 'highlights' && (
                <div style={{ display: 'grid', gridTemplateColumns: 'minmax(200px,1fr) 1fr', gap: 40 }} className="pdt-table-grid">
                  <div>
                    <h2 style={{ fontWeight: 800, color: 'var(--forest)', marginBottom: 14, fontSize: '0.95rem' }}>Product Highlights</h2>
                    {product.features && Array.isArray(product.features) && product.features.length > 0 ? (
                      product.features.map((f: string) => (
                  ...
                  <div>
                    <h2 style={{ fontWeight: 800, color: 'var(--forest)', marginBottom: 14, fontSize: '0.95rem' }}>Specifications</h2>
                    {[
              ...
              {activeTab === 'description' && (
                <div style={{ maxWidth: 680 }}>
                  <h2 style={{ fontWeight: 800, color: 'var(--forest)', marginBottom: 16, fontSize: '1.1rem' }}>About this Product</h2>
                  {product.description && (
              ...
              {activeTab === 'faq' && (
                <div style={{ maxWidth: 680 }}>
                  <h2 style={{ fontWeight: 800, color: 'var(--forest)', marginBottom: 20, fontSize: '1.1rem' }}>Frequently Asked Questions</h2>
              ...
              {activeTab === 'tags' && (
                <div>
                  <h2 style={{ fontWeight: 800, color: 'var(--forest)', marginBottom: 16, fontSize: '1.1rem' }}>Product Tags</h2>
